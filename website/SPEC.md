...
3. **The Gap** — explicit build/fix, prove-operation, and review-over-time buckets plus the Type II period gap.
4. **The Machinery** — the ten territory cards (see above).
5. **The Criteria** — the 61-cell checkbox matrix (see above), mirroring [CHECKLIST.md](../CHECKLIST.md) content via hover.
6. **Footer** — last verify run time, count of `unknown` checks ("blind spots"), link to `/db`.
