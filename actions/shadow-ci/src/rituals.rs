...
//                   a declared previous release and origin/main) to
//                   compliance-archives. CC8.1.
//
...
pub fn run_access_review() -> Result<i32, String> {
    ...
    md.push_str(&section_or_err("GitHub org members", || {
        let v = gh_json(&[
            "api",
            &format!("orgs/{org}/members?role=member"),
            "--paginate",
        ])?;
        Ok(logins_table(&v, &filter))
    }));
    md.push_str(&section_or_err("Outside collaborators", || {
        let v = gh_json(&[
            "api",
            &format!("orgs/{org}/outside_collaborators"),
            "--paginate",
        ])?;
        Ok(logins_table(&v, &filter))
    }));
    ...
    md.push_str(&section_or_err(
        "Repo-level direct grants & deploy keys",
        || {
            ...
            for r in repos.as_array().unwrap_or(&vec![]) {
                ...
                if let Ok(collabs) = gh_json(&[
                    "api",
                    &format!("repos/{org}/{name}/collaborators?affiliation=direct"),
                    "--paginate",
                ]) {
                    for c in collabs.as_array().unwrap_or(&vec![]) {
                        let row = format!(
                            "| {name} | collaborator: {} | {} |",
                            c["login"].as_str().unwrap_or(""),
                ...
                if let Ok(keys) = gh_json(&["api", &format!("repos/{org}/{name}/keys")]) {
                    for k in keys.as_array().unwrap_or(&vec![]) {
                        let row = format!(
                            "| {name} | deploy key: {} | read_only: {} |",
                            k["title"].as_str().unwrap_or(""),
                            k["read_only"]
                        );
                        if matches_filter(&row, &filter) {
                            out.push_str(&row);
    ...
    for project in &gcp_projects {
        ...
        md.push_str(&section_or_err(
            &format!("GCP user-managed service-account keys — {project}"),
            || {
                let out = run(
                    "gcloud",
                    &[
    ...
    let stamp = if filter.is_some() {
        format!("user-{}", utc_date("%Y%m%d-%H%M%S"))
    } else {
        date.clone()
    };
    emit(
        "access-review",
        vec![(format!("evidence/{year}/{q}/access-review-{stamp}.md"), md)],
    )?;
    Ok(0)
}
...
pub fn run_mgmt_packet() -> Result<i32, String> {
    ...
    md.push_str(&section_or_err("Bypass merges this quarter", || {
        ...
        for h in hits.lines() {
            ...
            if let Some(datepart) = file
                .strip_suffix(".json")
                .and_then(|s| s.rsplit('-').next())
            {
                if datepart >= q_start.replace('-', "").as_str() {
                    t.push_str(&format!(
                        "- `{file}` — verify a linked incident ticket + backport PR exists\n"
                    ));
                }
            }
        }
    ...
    md.push_str(&section_or_err(
        "Incidents this quarter (label: incident)",
        || {
            let out = gh(&[
                "issue",
                "list",
    ...
    md.push_str(&section_or_err(
        "Open shadow regression tickets (label: shadow)",
        || {
            let out = gh(&[
                "issue",
                "list",
    ...
    emit(
        "management-review",
        vec![(
            format!("evidence/{year}/{q}/management-review-{date}.md"),
            md,
        )],
    )?;
    Ok(0)
...
pub fn run_release_record() -> Result<i32, String> {
    ...
    let from_ref = env_or("RELEASE_FROM", "origin/main~1");
    let to_ref = env_or("RELEASE_TO", "origin/main");
    let range = format!("{from_ref}..{to_ref}");
    let commits = git(&["log", &range, "--oneline", "--no-merges"])?;
    if commits.trim().is_empty() {
        println!("no commits in release range {range}; nothing to record");
        return Ok(0);
    }
    let full_log = git(&["log", &range, "--pretty=%s"])?;
    ...
    let stats = git(&["diff", "--shortstat", &from_ref, &to_ref]).unwrap_or_default();
    let release_sha = git(&["rev-parse", &to_ref])?;
    ...
    emit(
        ...
        vec![
            (
                format!("releases/release-{ts}.json"),
    ...
    println!(
        "release-{ts}: {} commits, {} PRs, {} tickets",
        commits.lines().count(),
        prs.len(),
        tickets.len()
    );
...
mod tests {
    ...
    fn quarter_math() {
        assert_eq!(
            quarter_of("2026-01"),
            ("2026".into(), "Q1".into(), "2026-01-01".into())
        );
        assert_eq!(
            quarter_of("2026-07"),
            ("2026".into(), "Q3".into(), "2026-07-01".into())
        );
        assert_eq!(
            quarter_of("2026-12"),
            ("2026".into(), "Q4".into(), "2026-10-01".into())
        );
    }
