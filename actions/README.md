...
- **Judgment half (markdown, `../agent/` + `../commands/`)** — scanning, setup, the daily shadow audit, and the developer workflow. Executed by Claude (or any capable LLM) reading the runbooks.
...
1. **Vendor the tooling** into the adopting repo:
   ...
   .shadow/site/          ← copy of website/app (the gauge; daily-verify renders it statically on GitHub)
   ...
   ```
2. **Secrets**: one LLM key — `ANTHROPIC_API_KEY` (recommended) or `GEMINI_API_KEY` or `OPENAI_API_KEY` (daily verify + shadow agent); `LINEAR_API_KEY` (optional — ticket verification), `SLACK_WEBHOOK_URL` (optional — bypass/merge alerts).
...
| `daily-verify.yml` | cron daily + manual | Claude headless on `agent/03-verify-compliance.md` — the full shadow audit, gauge recompute, regression tickets |
...
| **LLM, headless** (Claude recommended for judgment work; Gemini or Codex accepted — first configured key wins) | The daily verify runbook; composing interview questions from gathered evidence; parsing human answers into filed artifacts; drafting postmortems, tabletop scenarios, and the system description. Never approves anything. |
